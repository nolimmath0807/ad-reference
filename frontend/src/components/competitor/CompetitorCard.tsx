...
import { Trash2, Pencil, Activity, Clock } from "lucide-react";
...
export function CompetitorCard({ stats, onDeleted }: CompetitorCardProps) {
  ...
  const { brand, sources, total_ads, last_collected_at } = stats;

  const handleDelete = async () => {
    setIsDeleting(true);
    await api.delete(`/brands/${brand.id}`);
    toast.success(`"${brand.brand_name}" has been removed.`);
    setIsDeleting(false);
    ...
    <>
      ...
      >
        <div className="flex flex-col gap-4 p-5">
          {/* Header: brand name + source badges + status + delete */}
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to stop monitoring "{brand.brand_name}"? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
